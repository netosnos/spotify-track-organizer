...
def get_reccobeats_info(spotify_tracks: list) -> tuple:
  ...
  for i in range(0, len(spotify_tracks), batch_size):
    ...
    try:
      ...
      print(
        f"\rProcessing batch {current_batch}/{total_batches} ({(current_batch/total_batches)*100:.1f}%)",
        end='')
      ...
      received_ids = {
        track.get('href', '').split('/')[-1]
        for track in tracks if track.get('href')
      }
      for track in batch:
        if track['id'] not in received_ids:
          ...
          racco_track = next(
            (t for t in tracks
             if t.get('href', '').split('/')[-1] == track['id']), None)
          if racco_track:
...
def save_results(tracks_with_ids: list, tracks_without_ids: list):
  ...
  output_with_ids = {**metadata, 'tracks': tracks_with_ids}
  ...
  if tracks_without_ids:
    output_without_ids = {**metadata, 'tracks': tracks_without_ids}
