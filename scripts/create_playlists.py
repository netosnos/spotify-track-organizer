...
class PlaylistCreator:
    def __init__(self, dry_run: bool = True):
        ...
        self.playlists = {
            'Chill Vibes': 'Soft, mellow, and relaxing tracks. Ideal for winding down or background ambiance.',
            'Feel-Good': 'Happy, upbeat, and energizing songs that lift your mood.',
            'Sad & Moody': 'Emotional, introspective, or melancholic songs. For reflective moments or sad vibes.',
            'Party Mode': 'High-energy, danceable tracks. Perfect for getting the party started.',
            'Training & High Energy': 'Fast-paced, energetic tracks to keep you moving during runs or cardio sessions.',
            'Driving Mix': 'Songs with a balanced, rhythmic feel—great for road trips or long drives.',
            'Other': 'A catch-all playlist for songs that don\'t have audio features or genre information.'
        }

        # Genre-based classification
        self.GENRES_BY_PLAYLIST = {
            'Chill Vibes': [
                'soft pop', 'acoustic pop', 'singer-songwriter', 'latin alternative', 'neo soul', 'latin r&b', 'folk', 'folk rock', 'indie folk', 'quiet storm', 'timba', 'bolero', 'bossa nova', 'latin jazz', 'jazz', 'huayno', 'trova', 'nueva trova', 'vocal jazz', 'adult standards', 'musicals'
            ],
            'Sad & Moody': [
                'sad sierreño', 'sierreño', 'latin folk', 'trova', 'nueva trova', 'norteño', 'ranchera', 'corridos bélicos', 'corridos tumbados', 'corrido', 'grupera', 'ballad', 'huayno', 'bolero', 'flamenco', 'flamenco pop', 'flamenco urbano'
            ],
            'Feel-Good': [
                'latin pop', 'colombian pop', 'pop', 'pop rock', 'pop urbano', 'soft pop', 'cumbia', 'cumbia norteña', 'vallenato', 'bachata', 'salsa', 'merengue', 'música mexicana', 'pagode baiano', 'forró', 'funk', 'funk pop', 'motown', 'tropical house', 'r&b', 'soul'
            ],
            'Party Mode': [
                'reggaeton', 'urbano latino', 'trap latino', 'argentine trap', 'rkt', 'dembow', 'edm', 'electro house', 'dancehall', 'electrocumbia', 'electro corridos', 'techengue', 'turreo', 'latin dance', 'latin hip hop', 'latin afrobeats', 'latin', 'reggaeton chileno', 'reggaeton mexa'
            ],
            'Training & High Energy': [
                'trap', 'rap', 'hip hop', 'trap latino', 'argentine trap', 'rock', 'hard rock', 'punk', 'pop punk', 'metal', 'nu metal', 'electro house', 'electro corridos', 'progressive house', 'latin rock', 'rock en español', 'mexican rock', 'alternative metal', 'progressive trance'
            ],
            'Driving Mix': [
                'rock en español', 'latin rock', 'rock', 'classic rock', 'soft rock', 'mexican rock', 'argentine rock', 'pop rock', 'country', 'country rock', 'americana', 'outlaw country', 'roots rock', 'britpop', 'new wave', 'aor', 'yacht rock', 'synthpop'
            ]
        }

    def is_chill_vibes(self, features: Dict) -> bool:
        """Check if track matches Chill Vibes criteria"""
        return (features['valence'] <= 0.5 and
                features['energy'] <= 0.5 and
                features['acousticness'] >= 0.3 and
                features['tempo'] <= 110 and
                features['danceability'] <= 0.7)

    def is_sad_moody(self, features: Dict) -> bool:
        """Check if track matches Sad & Moody criteria"""
        return (features['valence'] <= 0.4 and
                features['energy'] <= 0.6 and
                features['acousticness'] >= 0.2 and
                features['tempo'] <= 120)

    def is_feel_good(self, features: Dict) -> bool:
        """Check if track matches Feel-Good criteria"""
        return (features['valence'] >= 0.6 and
                features['energy'] >= 0.5 and
                features['danceability'] >= 0.5 and
                85 <= features['tempo'] <= 140 and
                features['acousticness'] <= 0.5)

    def is_party_mode(self, features: Dict) -> bool:
        """Check if track matches Party Mode criteria"""
        return (features['energy'] >= 0.6 and
                features['danceability'] >= 0.7 and
                features['valence'] >= 0.5 and
                features['tempo'] >= 100 and
    ...
    def is_training_high_energy(self, features: Dict) -> bool:
        ...
        return (features['energy'] >= 0.75 and
    ...
    def is_driving_mix(self, features: Dict) -> bool:
        ...
        return (90 <= features['tempo'] <= 150 and
                0.5 <= features['energy'] <= 0.9 and
                features['danceability'] >= 0.5 and
                features['acousticness'] <= 0.5)
    ...
    def classify_track(self, features: Dict) -> str:
        ...
        for name, conds in rules:
            if all(check_condition(features.get(f, 0), op, val) for f, op, val in conds):
                return name
        ...
        max_matches = -1
        for name, conds in rules:
            matches = sum(check_condition(features.get(f, 0), op, val) for f, op, val in conds)
        ...
        if len(best_playlists) == 1:
            ...
            for name, conds in rules:
                if name in best_playlists:
                    total_diff = sum(diff_condition(features.get(f, 0), op, val) for f, op, val in conds)
    ...
    def load_tracks(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        with open(self.no_features_file, 'r') as f:
            ...
            print(f"Found {len(tracks_without_features)} tracks without audio features")
        
        self.total_tracks = len(tracks_with_features) + len(tracks_without_features)
    ...
    def classify_track_by_genre(self, track: Dict) -> str:
        ...
        # Priority order
        for playlist in [
            'Chill Vibes', 'Sad & Moody', 'Feel-Good', 'Party Mode', 'Training & High Energy', 'Driving Mix']:
    ...
    def run(self):
        ...
        try:
            ...
            skipped_tracks = [track.get('name', 'Unknown') for track in tracks_without_features if not track.get('spotify_id')]
            if skipped_tracks:
            ...
            other_tracks = [track for track in tracks_without_features if self.get_track_uri(track, use_id=False) in other_uris]
            if other_tracks:
